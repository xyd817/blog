            ...
            <el-main>
                <div class="me-view-card">
                    ...
                    <div class="me-view-info">
                        ...
                        <el-button
                        @click="editArticle()"
                        style="position: absolute; left: 60%; margin-top:-25px"
                        size="mini"
                        round
                        icon="el-icon-edit"
                        >
                    ...
                    <div class="me-view-tag">
                        ...
                        v-for="t in article.tags" :key="t.id" round plain>
                            {{t.tagName}}
                        </el-button>
                    </div>
...
export default {
    ...
    computed: {
        avatar() {
            ...
            if (avatar && avatar.length && avatar.length > 0) {
                return avatar;
            }
            
        },
        title() {
            return `${this.article.title} - 文章 - 德哥Blog`
        }
    },
    components: {
    ...
    methods: {
        editArticle() {
            this.$router.push({path: `/write/${this.article.id}`, query: {isEdit: true}});
        },
        ...
        getArticle() {
            ...
            viewArticle(that.$route.params.id).then(res => {
                ...
                that.getCommentsByArticle(that.article.id);
            }).catch(error => {
                if (error !== 'error') {
